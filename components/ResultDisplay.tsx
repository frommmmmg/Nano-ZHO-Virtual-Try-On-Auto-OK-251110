...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ content, onUseImageAsInput, onImageClick, originalImageUrl }) => {
  ...
  const handleDownload = () => {
    ...
    const filename = content.originalFilename 
      ? `generated_${content.originalFilename}` 
  ...
  const handleDownloadBoth = () => {
    ...
    if (content.secondaryImageUrl) {
        const lineArtFilename = content.originalFilename ? `line-art_${content.originalFilename}` : `line-art_${fallbackName}`;
        downloadImage(content.secondaryImageUrl, lineArtFilename);
    }
    if (content.imageUrl) {
        const finalFilename = content.originalFilename ? `final-result_${content.originalFilename}` : `final-result_${fallbackName}`;
        downloadImage(content.imageUrl, finalFilename);
    }
  };
  ...
  const handleDownloadComparison = useCallback(async () => {
    ...
    const filename = content.originalFilename
        ? `comparison_${content.originalFilename}`
        : `comparison-image-${Date.now()}.png`;

    downloadImage(canvas.toDataURL('image/png'), filename);

  }, [originalImageUrl, content.imageUrl, content.secondaryImageUrl, content.originalFilename]);
  ...
  if (content.videoUrl) {
    const handleDownloadVideo = () => {
      const fallbackName = `generated-video-${Date.now()}.mp4`;
      const filename = content.originalFilename 
        ? `generated_${content.originalFilename.split('.')[0]}.mp4`
        : fallbackName;
      downloadImage(content.videoUrl!, filename);
    };
