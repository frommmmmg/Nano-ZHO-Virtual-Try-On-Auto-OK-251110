...
interface MultiImageUploaderProps {
    onPrimarySelect: (files: FileList) => void;
...
const MultiImageUploader: React.FC<MultiImageUploaderProps> = ({
    ...
    const handleSecondarySelectWrapper = (files: FileList) => {
      if (files.length > 0) {
        const file = files[0];
        const reader = new FileReader();
        reader.onload = (e) => onSecondarySelect(file, e.target?.result as string);
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <UploaderBox
                ...
                onImageSelect={handleSecondarySelectWrapper}
